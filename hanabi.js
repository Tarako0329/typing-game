...
function init() {
  ...
  canvas.width = document.getElementById('canv_div').clientWidth - Number(10);
...
function clickCanvas(evt) {
  let x = Math.random() * Number(canvas.width);
  let y = Math.random() * Number(canvas.height);
  addSparkles(Math.random() * 500 + 200 | 0, stage.mouseX, stage.mouseY, 1);
}
var typeSuccess = () => {
  let x = Math.random() * Number(canvas.width);
  let y = Math.random() * Number(canvas.height);
  addSparkles(Math.random() * 500 + 200 | 0, x, y, 1);
}
// sparkle trail
